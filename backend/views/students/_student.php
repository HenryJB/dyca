  ...
  <?php if(count($model)>0) :?>
    ...
    <div class="card mb-3">
      ...
      <div class="card-body">
        ...
        <?php $state = $get_state['name'] !=NULL ?$get_state['name'] :'Not Avaialable';?>

        <?php $get_country = Country::find()->where(['id'=>$model->country])->one();?>

        <?php $country = !empty($get_country['name'])|| $get_country['name'] !==NULL ?$get_country['name'] :'Not Avaialable';?>
          
        <?php $get_course = Course::find()->where(['id'=>$model->first_choice])->one();?>

        <?php $course =  !empty($get_course['name'])|| $get_course['name'] !==NULL ?$get_course['name'] :'Not Avaialable';?>
